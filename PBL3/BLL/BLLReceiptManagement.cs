...
namespace PBL3.BLL
{
    internal class BLLReceiptManagement
    {
        ...
        {
            string ID = (QLNS.Instance.Receipts.Any() ? QLNS.Instance.Receipts.OrderByDescending(p => p.ReceiptID.Length).ThenByDescending(p => p.ReceiptID).First().ReceiptID : "rpt0");
