...
namespace PBL3.BLL
{
    public class BLLProductManagement
    {
        ...
        {
            string product = (QLNS.Instance.Products.Any() ? QLNS.Instance.Products.OrderByDescending(p => p.ProductID.Length).ThenByDescending(p => p.ProductID).First().ProductID : "p0");
